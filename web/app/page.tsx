...
import { fetchCases, fetchCase, startOptimize, fetchRun, ensurePatient } from "../lib/api";
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (dragging === "left") {
        ...
        setLeftWidth(Math.min(Math.max(newW, 220), 420));
      } else if (dragging === "right") {
        const newW = rect.right - e.clientX;
        setRightWidth(Math.min(Math.max(newW, 260), 520));
      }
    };
  ...
  return (
    <div className={styles.page}>
      ...
      <div
        ...
        style={{ gridTemplateColumns: `${leftWidth}px 8px 1fr 8px ${rightWidth}px` }}
        ref={gridRef}
      >
        ...
        <div className={styles.right}>
          <DoseViewer dose={latestRun?.dose} structures={caseQuery.data?.structures || []} caseId={selectedCase} />
        </div>
      </div>
    </div>
  );
